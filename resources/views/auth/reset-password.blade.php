<x-guest-layout>
    ...
    <div class="mb-8 text-center">
        <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100">{{ __('Reset password') }}</h1>
        <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">{{ __('Choose a new password for your account.') }}</p>
    </div>

    <form method="POST" action="{{ route('password.store', ['locale' => (request()->route('locale') ?? app()->getLocale())]) }}" x-data="{ showPassword: false, showPasswordConfirm: false }">
        @csrf
        ...
        <div class="mt-4">
            ...
            <div class="relative mt-1">
                <x-text-input id="password" class="block w-full pr-12" ::type="showPassword ? 'text' : 'password'" name="password" required minlength="8" autocomplete="new-password" />
                <button
                    type="button"
        ...
        <div class="mt-4">
            ...
            <div class="relative mt-1">
                <x-text-input id="password_confirmation" class="block w-full pr-12" ::type="showPasswordConfirm ? 'text' : 'password'" name="password_confirmation" required minlength="8" autocomplete="new-password" />
                <button
                    type="button"
                    class="absolute inset-y-0 right-2 my-2 inline-flex items-center justify-center rounded-lg px-2 text-slate-500 hover:bg-blue-50 hover:text-slate-700 transition-colors dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200"
