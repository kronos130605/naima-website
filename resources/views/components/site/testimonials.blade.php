...
<section id="testimonials" class="{{
    ...
    <div class="mx-auto max-w-6xl px-4 {{ $isNewDesign ? 'relative z-10' : '' }}">
        <div class="{{ $isNewDesign ? 'text-center mb-16' : 'text-center mb-12' }}">
            ...
            <div class="mt-8">
                <a
                    ...
                    class="{{
                        $isNewDesign
                            ? 'inline-flex items-center justify-center rounded-2xl border-4 border-white bg-yellow-300 px-6 py-3 text-base font-black text-blue-900 hover:bg-yellow-400 transition-all hover:scale-110 shadow-lg'
                            : 'inline-flex items-center justify-center rounded-xl border border-blue-200 bg-white px-6 py-3 text-sm font-semibold text-blue-700 hover:bg-blue-50 hover:border-blue-300 transition-colors dark:bg-slate-950/70 dark:border-slate-800 dark:text-blue-300 dark:hover:bg-slate-900'
                    }}"
                >
        ...
        <div class="relative mb-12">
            <div class="{{ $isNewDesign ? 'overflow-y-auto max-h-[600px] pr-2 scrollbar-thin scrollbar-thumb-purple-400 scrollbar-track-purple-100 dark:scrollbar-thumb-purple-600 dark:scrollbar-track-slate-800' : '' }}">
                <div class="{{ $isNewDesign ? 'grid gap-8 md:grid-cols-2 lg:grid-cols-3' : 'grid gap-6 md:grid-cols-2 lg:grid-cols-3' }}">
                    @foreach(($testimonials['items'] ?? []) as $t)
                        <div class="{{
                            $isNewDesign
                                ? 'relative bg-white rounded-[2rem] shadow-2xl border-6 border-yellow-300 p-8 hover:shadow-purple-500/40 transition-all hover:-translate-y-3 hover:rotate-1 dark:bg-slate-900 dark:border-purple-400'
                            ...
                            <div class="{{ $isNewDesign ? 'flex items-center gap-4 border-t-4 border-yellow-200 pt-4 dark:border-purple-600' : 'flex items-center gap-3 border-t border-blue-50 pt-4 dark:border-slate-800' }}">
                                ...
                                <div>
                                    <p class="{{ $isNewDesign ? 'font-bold text-blue-600 text-base dark:text-blue-400' : 'font-semibold text-slate-900 text-sm dark:text-slate-100' }}">{{ $t['name'] }}</p>
                                    @if(!empty($t['role']))
                                        <p class="{{ $isNewDesign ? 'text-sm text-purple-600 font-medium dark:text-purple-400' : 'text-xs text-slate-500 dark:text-slate-400' }}">{{ $t['role'] }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        @auth
            <div class="{{
                ...
                <h3 class="{{ $isNewDesign ? 'text-2xl font-extrabold text-blue-600 mb-6 dark:text-blue-400' : 'text-xl font-semibold text-slate-900 mb-6 dark:text-slate-100' }}">
