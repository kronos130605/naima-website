<x-site-layout :title="__('site.mind_maps.page_title') . ' — FrenchBoost'" :brand="$brand" :cta="$cta" :locale="$locale" :locales="$locales">
    ...
    <section
        ...
        x-data="{ modal: null, openModal(m){ this.modal=m; document.body.style.overflow='hidden'; }, closeModal(){ this.modal=null; document.body.style.overflow=''; } }"
    ...
    >
        <div class="mx-auto max-w-6xl">
            ...
            <div class="flex flex-wrap gap-2 mb-8">
                @foreach($groups as $group)
                    <a
                        ...
                        class="rounded-full px-4 py-2 text-sm font-semibold transition-all border {{ $current_group === $group['key']
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                @forelse($maps as $map)
                    @php
                        $d = ['level'=>$map->level,'topic_en'=>$map->topic_en,'topic_fr'=>$map->topic_fr,'title_en'=>$map->title_en,'title_fr'=>$map->title_fr,'description_en'=>$map->description_en,'description_fr'=>$map->description_fr,'preview'=>$map->preview_image,'file'=>$map->file_path];
                    @endphp
                    <div class="group relative flex flex-col bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all overflow-hidden dark:bg-slate-800 dark:border-slate-700">
                        <div class="aspect-[4/3] bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center cursor-pointer relative" @click="openModal({{ Js::from($d) }})">
                            @if($map->preview_image)
                                <img src="{{ Storage::url($map->preview_image) }}" alt="{{ $map->{'title_'.$lang} }}" class="w-full h-full object-cover">
                            @else
                                <div class="text-center p-6"><div class="text-5xl mb-2">🗺</div><p class="text-xs text-slate-400 dark:text-slate-500">{{ __('site.mind_maps.preview_soon') }}</p></div>
                            @endif
                        ...
                        <div class="flex flex-col flex-1 p-4 gap-2">
                            ...
                            <div class="flex gap-2 mt-1">
                                <button type="button" class="flex-1 rounded-lg border border-blue-200 py-1.5 text-xs font-medium text-blue-700 hover:bg-blue-50 transition-colors dark:border-blue-800 dark:text-blue-400 dark:hover:bg-blue-950/30" @click="openModal({{ Js::from($d) }})">{{ __('site.mind_maps.view') }}</button>
                                @if($map->file_path)
                                    <a href="{{ Storage::url($map->file_path) }}" target="_blank" download class="flex-1 rounded-lg bg-blue-600 py-1.5 text-center text-xs font-medium text-white hover:bg-blue-700 transition-colors">{{ __('site.mind_maps.download') }}</a>
                                @else
                                    <span class="flex-1 rounded-lg bg-slate-100 py-1.5 text-center text-xs text-slate-400 dark:bg-slate-700 dark:text-slate-500">{{ __('site.mind_maps.coming_soon') }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
