<x-site-layout :title="__('site.page_title.testimonials')" :brand="$brand" :cta="$cta" :locale="$locale" :locales="$locales">
    ...
    <section class="bg-gradient-to-br from-yellow-100 via-purple-50 to-blue-100 py-20 relative overflow-hidden dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
        ...
        <div class="mx-auto max-w-6xl px-4 relative z-10">
            <div class="text-center mb-16">
                <h1 class="text-4xl md:text-5xl font-extrabold text-purple-600 mb-4 dark:text-purple-400" style="text-shadow: 2px 2px 0px rgba(59, 130, 246, 0.3);">
            ...
            <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3 mb-16">
                @forelse($posts as $post)
                    <div class="relative bg-white rounded-[2rem] shadow-2xl border-6 border-yellow-300 p-8 dark:bg-slate-900 dark:border-purple-400">
                        ...
                        <div class="flex items-center gap-4 border-t-4 border-yellow-200 pt-4 dark:border-purple-600">
                            ...
                            <div>
                                <p class="font-bold text-blue-600 text-base dark:text-blue-400">{{ $post->name }}</p>
                                @if(!empty($post->role))
                                    <p class="text-sm text-purple-600 font-medium dark:text-purple-400">{{ $post->role }}</p>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            <div id="submit" class="scroll-mt-24">
                @auth
                    <div class="max-w-2xl mx-auto bg-white rounded-[2rem] shadow-2xl border-6 border-blue-300 p-8 dark:bg-slate-900 dark:border-blue-600">
                        <h2 class="text-2xl font-extrabold text-blue-600 mb-6 dark:text-blue-400">
                ...
                @else
                    <div class="max-w-2xl mx-auto bg-white rounded-[2rem] shadow-2xl border-6 border-blue-300 p-8 text-center dark:bg-slate-900 dark:border-blue-600">
                        <h2 class="text-2xl font-extrabold text-blue-600 mb-2 dark:text-blue-400">{{ __('Share Your Experience') }}</h2>
                        <p class="text-slate-700 dark:text-slate-200 mb-6">{{ __('Log in to submit a testimonial.') }}</p>
                        <a href="{{ route('login', ['locale' => $locale]) }}" class="inline-flex items-center justify-center rounded-2xl border-4 border-white bg-yellow-300 px-5 py-2.5 text-base font-black text-blue-900 hover:bg-yellow-400 transition-all hover:scale-110 shadow-lg">
                            {{ __('Log in') }}
                        </a>
                    </div>
                @endauth
